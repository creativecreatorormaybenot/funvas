 ...
 */
class OpenSimplex2F {
	static const PSIZE = 2048;
	static const PMASK = 2047;

	OpenSimplex2F(int seed) {
		final source = Int16List(PSIZE);
      ...
      source[i] = i;
		...
		for (int i = PSIZE - 1; i >= 0; i--) {
          ...
          // same size, however, JavaScript (when running on web) does not allow
          // for int literals this big, which is why we need to use int.parse.
          * int.parse('6364136223846793005') + int.parse('1442695040888963407');
			var r = (seed + 31) % (i + 1);
			if (r < 0) r += i + 1;
			perm[i] = source[r];
  ...
  final perm = Int16List(PSIZE);
  final permGrad2 = List.filled(PSIZE, const Grad2(0, 0));
  final permGrad3 = List.filled(PSIZE, const Grad3(0, 0, 0));
  final permGrad4 = List.filled(PSIZE, const Grad4(0, 0, 0, 0));
...
class LatticePoint2D {
  const LatticePoint2D(this.xsv, this.ysv): dx = -xsv - ((xsv + ysv) * -0.211324865405187), dy = -ysv - ((xsv + ysv) * -0.211324865405187);
